...
export async function main() {
  ...
  const grade = Boolean(values.grade);
  const whole = existsSync(eventsPath) ? readFileSync(eventsPath, "utf8") : "";
  const offset = grade ? Math.max(0, whole.lastIndexOf('{"at"', whole.lastIndexOf('"type":"registered"'))) : whole.length;
  ...
  try {
    ...
    if (!grade) await input.question("After starting the capped broker and sandbox Claude, press Enter for preflight (voice still asleep): ");
    if (!grade) {
    ...
    const heard = (await input.question(grade ? "Was the approved non-canary result audible during the run? Type yes then Enter: " : "When you have FINISHED speaking, approving, and running hotmic sleep, answer: was the approved non-canary result audible? Type yes then Enter: ")).trim() === "yes";
    if (!grade) { const token = readFileSync(join(dir, "control.token"), "utf8").trim(); await control(join(dir, "hotmic.sock"), token, "sleep"); }
    ...
    const held = results.find(r => r.text.includes(grade ? "CANARY-" : canary));
    ...
    const checks = {
      five_replied: [...delivered].filter(id => ack.has(id) && replied.has(id) && id !== `${held?.request_id}:${held?.revision}`).length >= 5,
      canary_held: !!held && events.some(e => e.type === "egress" && e.request_id === held.request_id && e.allowed === false && e.reason === "release required"),
      zero_canary_outbound: !events.filter(e => e.type === "outbound").map(e => e.frame.content).join("").includes(grade ? "CANARY-" : canary),
